...
namespace Magnum
{
	namespace Shaders
	{
		...
		)
		{
...
#ifndef MAGNUM_TARGET_GLES
			const GL::Version version = context.supportedVersion({GL::Version::GL460, GL::Version::GL320, GL::Version::GL310, GL::Version::GL300, GL::Version::GL210});
#else
			...
			if (!((flags & (~Flag::Shadows)) >= Flag::UniformBuffers) && lightCount)
			{
			...
			{
				vert.addSource("#define HAS_SHADOWS\n")
			...
			if (flags & Flag::Shadows)
			{
				frag.addSource("#define HAS_SHADOWS\n")
					.addSource("#define NUM_SHADOW_MAP_LEVELS " + std::to_string(numShadowLevels) + "\n");
			}
...
/* ES3 has this done in the shader directly and doesn't even provide
   bindFragmentDataLocation() */
		...
		PhongGLShadows::PhongGLShadows(CompileState &&state) : PhongGLShadows{static_cast<PhongGLShadows &&>(std::move(state))}
		{
			...
			{
#ifndef MAGNUM_TARGET_GLES2
				if ((_flags & (~Flag::Shadows)) >= Flag::UniformBuffers)
				{
					if (_drawCount > 1)
...
/* Set defaults in OpenGL ES (for desktop they are set in shader code itself) */
...
#endif
			{
				...
				if (_lightCount)
				{
					...
					flags &Flag::Shadows ? setTextureMatrix(Matrix3{Math::IdentityInit});
